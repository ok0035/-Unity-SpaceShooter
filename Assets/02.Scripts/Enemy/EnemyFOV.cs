...
public class EnemyFOV : MonoBehaviour {
    ...
    private Vector3 offsetUp = new Vector3(0, 1.5f, 0);
    ...
    public bool isViewPlayer()
    {
        ...
        //적 캐릭터와 주인공 사이의 방향 벡터를 계싼
        dir = (playerTr.position - enemyTr.position).normalized;

        //레이캐스트를 투사해서 장애물이 있는지 여부를 판단
        if(Physics.Raycast(enemyTr.position + offsetUp, dir, out hit, viewRange, layerMask))
        {
    ...
    public void Update()
    {
        Debug.DrawRay(enemyTr.position + offsetUp, dir * viewRange, Color.green);
    }
